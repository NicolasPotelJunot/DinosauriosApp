...
export const CardCaracterist = ({ dino }) => {
  ...
  return (
    <article className="bg-white px-3 pt-5 pb-10 rounded-2xl transition-all duration-300 shadow-gray-400 shadow-md">
      ...
      <div className="grid">
        <p className="text-lg">Longitud: {longitud}</p>
        <p className="text-lg">Altura: {altura}</p>
        <p className="text-lg">Peso: {peso}</p>
        <p className="text-lg">Período: {era}</p>
        <p className="text-lg">Dieta: {tipo}</p>
        <p className="text-lg">Descrito por: {autor}</p>
      </div>
    </article>
  );
};
